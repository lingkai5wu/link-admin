...
import type { MenuOrderUpdateDTO } from '@/types/api/query'
import type { MenuTreeOption } from '@/types/menu'
...
let oldOptions: MenuTreeOption[] = []
const options = ref<MenuTreeOption[]>()
getOptions()
...
async function getOptions() {
  ...
  try {
    ...
    oldOptions = generateTreeOptions(menuVOs)
...
function findSiblingsAndIndex(
  ...
  for (let i = 0; i < nodes.length; ++i) {
    const siblingNode = nodes[i] as MenuTreeOption
...
function generateMenuOrderUpdateDTOs(treeNodes: MenuTreeOption[] | undefined) {
  if (!treeNodes) {
    return []
  }
  const result: MenuOrderUpdateDTO[] = []
  let sortOrder = 0

  function traverse(node: MenuTreeOption, pid: number): void {
    sortOrder++
    const dto: MenuOrderUpdateDTO = {
      id: node.id,
      ...(pid !== node.pid && { pid }),
...
async function handleClick() {
  const dtos = generateMenuOrderUpdateDTOs(options.value)
  ...
  try {
    await batchUpdateMenuOrder(dtos)
  } finally {
